...
## 2) Create a NEW SSH key on Windows (for a new PC)

Open **PowerShell**:

### 2.1 Generate key
```powershell
ssh-keygen -t ed25519 -a 64 -f $env:USERPROFILE\.ssh\freedomvpn_ed25519
```

### 2.2 Copy the public key
```powershell
type $env:USERPROFILE\.ssh\freedomvpn_ed25519.pub
```
